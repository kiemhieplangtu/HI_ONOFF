...
phi_v = cal_phi_v(test_v, v0, wid)

if(False):
    plt.plot(test_v/constants.kilo, phi_v[0])
    plt.show()
    sys.exit()
...
tau_cnm = tau_vel(nu0, NCNM, Tex, phi_v) + addnoise(ncnm, nchan)
...
NWNM     = NWNM * 1.e4 ## To m^-2  http://www.ucolick.org/~xavier/AY230/ay230_HI21cm.pdf
tau_wnm  = tau_vel(nu0, NWNM, Tex_wnm, phi_v) #+ addnoise(nwnm, nchan)
Toff_wnm = cal_Toff_WNM(tau_wnm, Tbg, Tex_wnm)


if(False):
    plt.plot(test_v/1000., Toff_wnm[0])
    plt.ylabel('TOFF')
    plt.show()
...
# Ton_vel  = cal_Ton(tau_vel, Tbg, Tc, Tex)
...
# axs[0].plot(test_v/1000., tau_wnm[0])
axs[0].set_ylabel('T_OFF: Texp [K]')
# axs[0].set_xlim(0, 2)
# axs[0].grid(True)

axs[1].plot(test_v/1000., np.exp(-tausum))
axs[1].set_ylabel('T_ON: e(-tau)')
axs[1].set_xlabel('Vel [km/s]')
